...
import type { DayRole, Race } from './types';

export type Variant = 'progression-run' | 'goal-pace-tempo' | 'race-pace-ladder' | 'race-week-road' | 'hm-simulation';

/** Goal/10K/5K pace for the workout menus (seconds per mile). */
...
export function hardWorkoutContent(variant: Variant, weekIndex: number, roadPaces?: RoadPaces): { title: string; description: string } {
  switch (variant) {
...
export function longContent(miles: number): { title: string; description: string } {
  const pct = Math.round(LONG_RUN_PCT_OF_WEEKLY * 100);
  return { title: 'Long Run', description: `${miles} mi long run (about ${pct}% of weekly mileage). Keep it conversational. ${STRETCH_NOTE}` };
}
...
export const ROLE_EXPLANATION: Record<DayRole, string> = {
  ...
  hard: 'Hard days are where fitness actually gets built: progression runs, goal-pace tempo intervals, or race-pace ladder work run at a genuinely demanding effort. The plan schedules exactly two of these a week and surrounds them with recovery, because the adaptation happens during rest, not during the workout itself. Warm up and cool down properly, hit the effort or pace called for, and if a rep starts falling apart, it is fine to ease off rather than force it.',
  ...
  long: `The long run is the week's single biggest dose of aerobic development, built up gradually to roughly ${Math.round(LONG_RUN_PCT_OF_WEEKLY * 100)}% of your weekly mileage. Keep the effort conversational, especially in the first half — the point is time on your feet and durability, not speed. In the peak phase some long runs become "simulation" runs with a big goal-pace block in the middle, a dress rehearsal for race day.`,
  ...
  race: 'Race day is the payoff for the whole training cycle. Warm up the way you have practiced, settle into your goal pace early rather than sprinting the first mile, and trust the fitness the base, build, and peak phases were built to produce. However it goes, log how it felt afterward — that feedback shapes whatever comes next.',
};

export function contentForRole(
  role: DayRole,
  miles: number,
  opts: { variant?: Variant; weekIndex: number; roadPaces?: RoadPaces; withDrills?: boolean; race?: Race },
): { title: string; description: string } {
  switch (role) {
    case 'hard':
      return hardWorkoutContent(opts.variant ?? 'progression-run', opts.weekIndex, opts.roadPaces);
    case 'easy':
